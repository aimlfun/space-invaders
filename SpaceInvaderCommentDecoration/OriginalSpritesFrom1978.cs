...
namespace SpaceInvaderCommentDecoration;
...
public static class OriginalSpritesFrom1978
{
    ...
    {
        Sprite sprite = sprites[c.ToString()];
        ...
        {
            StringBuilder pixelRowAsText = new();

            for (int x = 0; x < sprite.WidthInPX; x++)
            {
                pixelRowAsText.Append( sprite.Pixels[x, y] == 1 ? "█" : " ");
            }

            sb.Add(pixelRowAsText.ToString());
        }
    ...
    public static string RenderTextInSpaceInvaderFont(string text, string prefix = "")
    {
        ...
        foreach (char c in text)
        {
            ...
            if (sprites.ContainsKey(c.ToString())) lines.Add(SpritePixelsByRow(c));
        }
