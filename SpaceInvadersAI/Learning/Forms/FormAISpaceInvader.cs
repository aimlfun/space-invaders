...
namespace SpaceInvadersAI
{
    ...
    public partial class FormSpaceInvaders : Form
    {
        ...
        private void StartToPlayOrLearn()
        {
            ...
            {
                Text = "Space Invaders AI - Playing";
        ...
        private void PictureBox_MouseClick(object? sender, EventArgs e)
        {
            ...
            int x = Cursor.Position.X - pictureBox.Left - this.Left - this.Padding.Left - pictureBox.Padding.Left - pictureBox.Margin.Left - this.DefaultMargin.Left;

            // translate that to a generation number
            generationClickedOn = (int)Math.Round((float)(x - SimpleGraph.Origin.X) / SimpleGraph.XToGeneration);

            // force a repaint of all the graphs, and the line will be drawn on each one
            foreach (PictureBox pictureBoxToRepaint in flowLayoutPanelGames.Controls)
            {
                pictureBoxToRepaint.Invalidate();
            }
        }
        ...
        private void PictureBox_Paint(object? sender, PaintEventArgs e)
        {
            ...
            int x = (int)Math.Round(SimpleGraph.Origin.X + SimpleGraph.XToGeneration * generationClickedOn);
        ...
        private static List<Bitmap> GetGraphsFromDataPoints(List<StatisticsForGraphs> dataPoints, string topPerformerInfo)
        {
            ...
            List<float> fitness = dataPoints.Select(x => x.FitnessScore).ToList(); // how fit the AI was
        ...
        private void FormSpaceInvaders_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                // quick learning mode
                case Keys.Q:
                    // the key Won't do anything if we are in playing mode...
                    if (PersistentConfig.Settings.Mode == PersistentConfig.FrameworkMode.playing) return;

                    if (!LearningController.InQuickLearningMode)
                    {
                        // unless you want a spurious error on wake-up/hot-reload, we need to do this...
                        RemoveAllPictureBoxesAndDisplayMessage("Please wait... Quick learning mode initiated - progress graphs will appear at the end of each generation."); 
                    }
                ...
                case Keys.P:
                    if (PersistentConfig.Settings.Mode == PersistentConfig.FrameworkMode.playing)
                    {
                        AIPlayController.Paused = !AIPlayController.Paused;
                    }
                    else
                    { 
                ...
                case Keys.R:
                    // SonarQube doesn't like this, but I disagree. This is the singular form that sets this.
                    DebugSettings.c_debugDrawMainRadar = !DebugSettings.c_debugDrawMainRadar;
                    break;
                
                case Keys.S:
                    // SonarQube doesn't like this, but I disagree. This is the singular form that sets this.
                    DebugSettings.c_debugDrawShieldRadar = !DebugSettings.c_debugDrawShieldRadar;
                    break;
            }
        }
